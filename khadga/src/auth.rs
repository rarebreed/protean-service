...
pub fn build_cookie(key: &str, val: &str, expires: Option<Duration>, flags: &[&str]) -> String {
    ...
    let final_cookie = flags.into_iter().fold(cookie.clone(), |mut acc, next| {
        acc = acc + "; " + &next;
        return acc;
    });
    final_cookie
...
pub async fn make_verify_request(args: (LoginParams, User)) -> Result<impl Reply, Infallible> {
    ...
    match pgdb::establish_connection("test_db").await {
        ...
        Err(e) => {
            let resp = builder
                .status(StatusCode::from_u16(500).unwrap())
                .body(format!(
                    "Unable to create connection to postgresql database: {}",
                    e
                ))
        ...
        Ok(resp) => {
            if resp.status() != 201 {
                ...
                match pgdb::lookup_user(&db_client, "users", &dbuser).await {
                    ...
                    Err(_) => {
                        // In this case, we haven't seen this user before, so let's add him
                ...
                match create_jwt(&user.user_name, &user.email) {
                    Ok(jwt) => {
                        let jwt_resp: JWTResponse =
                            serde_json::from_str(&jwt).expect("Could not deserialize");
                        ...
                        let cookie =
                            build_cookie("jwt", &jwt_resp.token, Some(duration), &http_only_flags);
                        ...
                        let khadga_user =
                            build_cookie("khadga_user", &username, Some(duration), &secure_flags);
