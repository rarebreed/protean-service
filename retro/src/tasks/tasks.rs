...
/// Modifiers can be either External or Internal
pub enum ModifierType {
    /// A modifier intrinsic to the character (eg, being wounded)
    Internal,
    /// A modifier that is external to the character (poor lighting)
    External,
}

/// Modifiers can either be helpful or hurtful
...
/// Defines modifiers on taks
pub struct TaskModifier<T>
where
    T: NumOps,
{
    /// The name of the modifier (eg Poor lighting)
    pub name: String,
    /// The value of the modifier
    pub magnitude: T,
    /// Whether the modifier is Positive or Negative
    pub direction: ModifierDirection,
    /// Whether the modifier is Internal or External
...
pub struct Task {
    ...
    pub t_type: TaskType,
    /// A vector of TaskModifiers for this task
    pub modifiers: Vec<TaskModifier<u32>>,
    ///  The time it normally takes to perform the task (seconds)
    pub time: f64,
}

/// Enumerats the different kinds of tasks
pub enum TaskType {
    /// Tasks which take several resolutions to complete over time
    Gradual,
